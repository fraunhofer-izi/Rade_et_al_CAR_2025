...
)
{
  ...
  if(category == "CD4" | category == "CD8") {
    mir = readRDS("data/signatures/mir_gene_set_collection.Rds")
    msig_df = mir[grepl("Chu", mir$TermID), ]
    msig_df = msig_df[grepl(category, msig_df$TermID), ]
    msig_df$TermID = gsub(paste0(category, "-"), "", msig_df$TermID)
    msig_df$TermID = gsub(" \\(Chu\\)", "", msig_df$TermID)
  ...
  if(ora == T){
    ...
    if(!is.null(msig_df$gs_exact_source)){
    ...
    if(is.character(genes)){
      genes = setNames(as.numeric(names(genes)), genes)
    }
    fgRes = fgsea::fgsea(pathways = msig_list, stats = genes)
    fgRes$pathway = gsub("HALLMARK_", "", fgRes$pathway)
    fgRes$pathway = gsub("INTERFERON", "IFN", fgRes$pathway)
    fgRes$pathway = gsub("EPITHELIAL_MESENCHYMAL_TRANSITION", "EMT", fgRes$pathway)
    fgRes = fgRes[order(fgRes$padj, decreasing = F), ]
  }
...
ora_bubble = function(
  ...
  gsea.df$pathway = gsub("ANTIGEN", "AG", gsea.df$pathway)
  gsea.df$pathway = gsub("POSITIVE_ALPHA_BETA_", "", gsea.df$pathway)
  ...
  if(!is.null(ftrs.list)) {
    ...
    if(is.null(max.value)){
      ...
      if(quantile.filter == T) {
        thres = quantile(gsea.df$zscore, .99)
        gsea.df$zscore[gsea.df$zscore > thres & !is.infinite(gsea.df$zscore)] = thres
        gsea.df$zscore[gsea.df$zscore < -thres & !is.infinite(gsea.df$zscore)] = -thres
        max.value = thres
      }
    } else {
  ...
  if(facet.spit == T) {
    gsea.df$FACET = gsub("_.*", "", gsea.df$MP)
    gsea.df$MP = gsub(".*_", "", gsea.df$MP)
    if("Late" %in% gsea.df$MP) {
      gsea.df$MP = gsub("Apheresis", "LP", gsea.df$MP)
      gsea.df$MP = factor(gsea.df$MP, levels = c("LP", "Late", "Very Late"))
    }
  }
  ...
  if(!is.null(ftrs.list)) {
    ...
    pl = pl + geom_point(
      ...
      scale_fill_gradientn(
        colors = scico(30, palette = 'navia', direction = -1),
        breaks = pretty_breaks(n = 3),
      )
  }
  pl = pl + mytheme(base_size = font.size) +
    theme(
      ...
      axis.text.x = element_text(angle=45, vjust=1.05, hjust=1, size = rel(1)),
...
ora_barpl = function(
  ...
  pl = ggplot(data = gsea.df, aes(x = pathway, y = X_AXIS)) +
    coord_flip()
  if(!is.null(ftrs.list)) {
    pl = pl + geom_bar(aes(fill = fill), stat = "identity",  linewidth = .2, width = bar.width) +
      scale_fill_scico(
        palette = "vik", midpoint = 0, begin = .1, end = .9,
        limits = c(-max.value, max.value),
