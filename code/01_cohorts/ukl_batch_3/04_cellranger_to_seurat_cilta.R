...
work.path = paste0(manifest$ukl_b3$data_dl, "cilta")
seurat.path = paste0(manifest$base$workdata, "cohorts/ukl_batch_3/seurat/")
dir.create(seurat.path, recursive = T)

obj.path = paste0(seurat.path, "seurat_ori_cilta.Rds")
...
se.meta = merge(seurat.l[[1]], y = seurat.l[2:length(seurat.l)], add.cell.ids = names(seurat.l), project = "ukl_batch3")
