...
fastqs = manifest$ukl_b3$fastq

ref.base = manifest$refs

ref.gex.ide = paste0(ref.base, "GRCh38-CiltaCel/")
...
ref.features = paste0(manifest$base$homes, "data/feature_reference_b3.csv")
out.dir = paste0(manifest$base$workdata, "cohorts/ukl_batch_3/cellranger/cilta/")
...
df$GEX_REF = ref.gex.ide

df = df[!duplicated(df$SAMPLE), ]
# df$PATH = paste0(df$PATH, "/", df$SAMPLE)
