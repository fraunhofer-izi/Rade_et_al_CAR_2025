...
seurat.path = paste0(manifest$ukl_b4$seurat)
out.path = paste0(seurat.path, "seurat_post_souporcell.Rds")

se.meta = readRDS(paste0(seurat.path, "seurat_ori.Rds"))
se.meta.ori = se.meta
se.meta = NormalizeData(se.meta)

pdata.assoc = read.csv2("data/metadata_basic_run_4_pub.csv", sep = ",")
pdata.assoc$LANE_NAME = gsub("_", "-", pdata.assoc$LANE_NAME)
colnames(pdata.assoc) = gsub("-", "_", colnames(pdata.assoc))
pdata.assoc$GENDER = ifelse(pdata.assoc$GENDER == "m", "MALE", "FEMALE")

souporcell.path = paste0(manifest$ukl_b4$work, "souporcell/output")
...
se.meta.ori = AddMetaData(se.meta.ori, sc)
se.meta.ori@meta.data$WELL = se.meta.ori@meta.data$orig.ident
se.meta.ori@meta.data$orig.ident = se.meta.ori@meta.data$sample_name
se.meta.ori@meta.data$orig.ident = factor(se.meta.ori@meta.data$orig.ident)
se.meta.ori@meta.data$sample_name = NULL
Idents(se.meta.ori) = "orig.ident"

table(is.na(se.meta.ori$orig.ident))
se.meta.ori = se.meta.ori[, !is.na(se.meta.ori$orig.ident)] # Cells that could not be assigned by souporcell are removed

# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# scDblFinder
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
obj.l = Split_Object(se.meta.ori, split.by = "orig.ident", threads = 40)
...
se.meta.ori = AddMetaData(se.meta.ori, scDb.res)
DefaultAssay(se.meta.ori) = "RNA"
Idents(se.meta.ori) = "orig.ident"
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
se.meta.ori
saveRDS(se.meta.ori, file = out.path)
