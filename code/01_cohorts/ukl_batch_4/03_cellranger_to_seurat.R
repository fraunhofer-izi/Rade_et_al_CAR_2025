...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
.cran_packages = c(
  "Seurat", "yaml", "dplyr", "doParallel", "parallel", "data.table", "Matrix"
)
.bioc_packages = c()
...
work.path = manifest$ukl_b4$data_dl
seurat.path = paste0(manifest$base$workdata, "cohorts/ukl_batch_4/seurat/")
...
se.meta = merge(seurat.l[[1]], y = seurat.l[2:length(seurat.l)], add.cell.ids = names(seurat.l), project = "ukl_batch4")
