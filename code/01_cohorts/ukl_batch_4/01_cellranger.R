...
fastqs = manifest$ukl_b4$fastq

ref.base = manifest$refs

# In Run 4 we have only cilta samples
ref.gex.cilta = paste0(ref.base, "GRCh38-CiltaCel/")
...
ref.features = paste0(manifest$base$homes, "data/feature_reference_b4.csv")
out.dir = paste0(manifest$base$workdata, "cohorts/ukl_batch_4/cellranger/")
...
# k = names(table(df$SAMPLE_SHORT)[table(df$SAMPLE_SHORT) == 8])
# k = c("T3-A", "T3-B", "T3-D")
# df = df[df$SAMPLE_SHORT %in% k, ]
# table(df$SAMPLE_SHORT)
