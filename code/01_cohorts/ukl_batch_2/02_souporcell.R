...
cellranger = paste0(manifest$ukl_b2$data_dl)
ref.fasta = paste0(manifest$refs, "reference_sources/Homo_sapiens.GRCh38.dna.primary_assembly.fa")

common.variants = paste0(manifest$base$workdata, "cohorts/ukl_batch_2/souporcell/common_variants_grch38.vcf")

out.dir = paste0(manifest$ukl_b2$work, "souporcell/output/")
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# sbatch
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
